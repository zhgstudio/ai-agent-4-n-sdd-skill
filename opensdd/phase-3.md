...
5. **只读约束**：读取被依赖模块的 `INTERFACE.md` 时，只允许读取接口定义，不允许读取 `INTERNALS.md` 中的内部实现细节
6. **不予许设计师写入 `AGENTS.md`**——设计师发现的模块专属约束写在 `INTERNALS.md` 中即可，只有全局性约束才进入 `AGENTS.md`，而这种约束在架构阶段已被定义
