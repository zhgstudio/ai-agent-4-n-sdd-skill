...
7. **实验性代码处理原则**：
   - 设计过程中如需临时推演接口可行性、验证数据流等，可将实验性代码放在项目根目录的 `tmp/` 下。`tmp/` 中的内容不进入版本管理（已在 `.gitignore` 中排除），用完即弃
   - 如果产出的是具有永久指导价值的示意代码或伪代码（如核心算法的伪码、接口调用的示例序列），应当直接嵌入 `INTERNALS.md` 的对应章节中，作为设计说明的一部分。不要放在独立的代码文件中

## 审查清单

人类评审每个模块设计时建议逐项核对以下清单：

- [ ] **INTERFACE.md — 模块概述与职责边界**：是否清晰说明了本模块做什么、不做什么
- [ ] **INTERFACE.md — 核心数据结构**：概念模型、实体字段、关系是否完备（不含 DDL）
- [ ] **INTERFACE.md — 接口定义**：每个接口的入参、出参、错误码是否完整
- [ ] **INTERNALS.md — 核心逻辑流程**：主流程/状态机是否描述清楚
- [ ] **INTERNALS.md — 内部实现细节**：关键算法、数据持久化策略等是否有说明
- [ ] **INTERNALS.md — 功能特性列表**：每条以 `### {NN}-F{NNN}` 编号，覆盖所有接口
- [ ] **命名一致性**：所有字段名、接口名是否继承 ARCHITECTURE.md 的规范
- [ ] **接口契约**：提供的接口签名与被依赖方在依赖矩阵中声明的需求是否一致
- [ ] **目录与编号**：模块目录名、编号是否与 ARCHITECTURE.md 模块引用表严格一致
- [ ] **只读约束**：是否未读取被依赖模块的 INTERNALS.md
